...
@router.post("/auth/admin/grant-full-access")
async def admin_grant_full_access(request: Request):
    """
    Temporary admin endpoint: grant a user full Pro-tier access without payment.
    Requires X-Admin-Password header.
    Sets tier=pro, subscription_status=active, period_end=+365d, bumps token_version.
    """
    import os, hmac
    ...
    admin_pw = os.environ.get("ADMIN_PASSWORD", "")
    header_pw = request.headers.get("X-Admin-Password", "")
    if not admin_pw or not header_pw or not hmac.compare_digest(admin_pw, header_pw):
        raise HTTPException(status_code=401, detail="Admin auth required")

    body = await request.json()
    email = (body.get("email") or "").strip().lower()
    tier = body.get("tier") or "pro"
    if tier not in ("free", "starter", "growth", "pro"):
    ...
    with db_cursor() as (conn, cur):
        cur.execute("SELECT id, email, tier, subscription_status FROM fsbo_users WHERE LOWER(email) = %s", (email,))
        row = cur.fetchone()
        if not row:
            raise HTTPException(status_code=404, detail=f"User not found: {email}")
        user_id = row["id"]
        prev_tier = row["tier"]
        prev_status = row["subscription_status"]
