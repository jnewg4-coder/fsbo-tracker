...
async def admin_grant_full_access(request: Request):
    """Temp admin endpoint — grant tier access. X-Admin-Password required."""
    import os, hmac
    ...
    with db_cursor() as (conn, cur):
        cur.execute("SELECT id, tier, subscription_status, subscription_period_end FROM fsbo_users WHERE LOWER(email) = %s", (email,))
        row = cur.fetchone()
        if not row:
        ...
        prev_end = row["subscription_period_end"]
